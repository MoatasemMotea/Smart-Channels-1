...
/**
 * D-065 — the 3D cover carousel on /gallery.
 *
...
test("arrows rotate the ring and the loop returns to the first item", async ({ page }) => {
  await page.goto("/en/gallery", { waitUntil: "networkidle" });
...
test("/gallery renders the carousel with no console errors in both locales", async ({ page }) => {
  for (const loc of ["en", "ar"]) {
    const errors: string[] = [];
    page.on("console", (m) => { if (m.type() === "error") errors.push(m.text()); });
    await page.goto(`/${loc}/gallery`, { waitUntil: "networkidle" });
    await expect(page.locator(".gcar")).toHaveCount(1);
    await expect(page.locator(".gallery-masonry-item")).toHaveCount(0);
