...
export interface GalleryItem {
  ...
  poster?: string;
  /** D-077: a smaller derivative for the carousel card (the lightbox keeps `src`). */
  thumb?: string;
  /** D-077: `object-position` for the card's cover crop when the centre crop cuts the subject (e.g. "50% 20%"). */
  focus?: string;
  caption?: LocalizedText;
  year?: number;
  location?: LocalizedText;
