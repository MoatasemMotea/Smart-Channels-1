...
| D078-RPM | The owner's deletion list named "RPM"; no record carries that id or name, so it was dropped (six deleted). The derived `response-plus` mark reads "RESPONSE PLUS MEDICAL COMPLEX — RPM" — RPM is that client's own monogram. | One client record more or less on the rail | Owner decision 2026-09-21: nothing deleted on an inference. **Closes with one line from the owner:** "RPM = response-plus, delete" (record + `response-plus.webp` + `response-plus.png`, 24 → 23) or "keep". |
...
| D078-HOVER-DARK-INK | On the dark theme, hover returns a mark's original colours (directive); 24 of 64 marks are dark-ink (luminance < 95) and become barely visible in that state — e.g. `hpe`'s black "HP" beside its green "E". The two `originalColor` black marks (`hopscotch`, `moments-international`) are in that state permanently on dark. | Hover legibility on dark only; rest state is unaffected | Recorded 2026-09-21, not acted on — the rule is the owner's. Options if wanted: a faint light plate behind the hovered mark only, or keeping the white filter on dark for marks flagged dark-ink. Either is a CSS-only change; the ink-luminance list is in the D-078 report. |
