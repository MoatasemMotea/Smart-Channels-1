...
| D067-OFFSCREEN-PLAY | On the homepage the carousel's centre video plays (muted) from mount, before the `#gallery` section scrolls into view — one ≈4 MB request on every homepage load. | Homepage weight; nothing visible | The D-065 carousel was moved as built by directive. Candidate fix: gate `play()` on an IntersectionObserver as the deleted preview did (a small change inside `GalleryCarousel`). Owner to decide. |
