...
- **Zero diff on the forbidden set** beyond the one authorised line: `GalleryLightbox.tsx`, Industries, Projects, products, `seo.ts`, `sitemap.ts`, `package.json` untouched. Naming: the file keeps the intake name `al-nassr-press-room` (the owner renamed only `3.jpeg`); the public text names the venue only.
