...
- **Tally:** 30 of 33 types photographed. On the placeholder: Mice, Keyboards (held — haze, awaiting the pushed replacements), PTT Radios (held — marketing scene, JPEG). The computing-category smoke test now expects four photographed and two held. Gates: `npm run validate`, `npm run check` (11), `npm run build`, `npm run test:smoke` (149 passed, 25 skipped).
