...
- **Linking:** 23 of 31 types carry an image; the image follows the TYPE (five Switches cards → `switches.webp`, five Media Converters cards → `media-converters.webp`). Eight types render the placeholder: Camera Mounts, Decoders, HDMI Splitters, Monitors, Mice, Keyboards (no image supplied), Desktop PCs and PTT Radios (held). No locked manifest; the validator's D-059 section keeps checking only that a referenced file exists.
...
| D064-HELD-IMAGES | `ptt-radios.jpeg` and `desktop-pcs.png` sit in `media-source/images/products-intake/`, tracked, unlinked. | PTT Radios and Desktop PCs cards show the placeholder | Owner decision 2026-09-13: the radios file is a marketing scene with captions, the PC file has a baked white ground. Resolve by a replacement cut-out (WebP/PNG with real alpha) dropped in the same intake folder — a one-field link on the type's row. Six more types have no image at all: Camera Mounts, Decoders, HDMI Splitters, Monitors, Mice, Keyboards. |
